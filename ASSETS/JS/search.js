...
function search(artist) {
    ...
    fetch(url + artist)
        ...
        .then((singleArtist) => {
            console.log("Il nome cercato è " + singleArtist.data[0].artist.name);
...
function displayArtist(singleArtist) {
    let relevantArtist = singleArtist.data[0];
    relevantName = relevantArtist.artist.name;
    relevantPic = relevantArtist.artist.picture;
